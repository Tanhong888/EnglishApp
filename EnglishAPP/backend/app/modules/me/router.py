...
from app.db.models import Article, UserArticleFavorite, UserReadingProgress, UserVocabEntry, Word
...
@router.get("/stats")
def me_stats(db: Session = Depends(get_db)) -> dict:
    read_articles = db.scalar(
        select(func.count(func.distinct(UserReadingProgress.article_id))).where(UserReadingProgress.user_id == DEMO_USER_ID)
    ) or 0
    ...
    vocab_count = db.scalar(select(func.count(UserVocabEntry.id)).where(UserVocabEntry.user_id == DEMO_USER_ID)) or 0
    ...
    if total_progressed > 0:
        completed = db.scalar(
            ...
            .where(UserReadingProgress.user_id == DEMO_USER_ID, Article.is_completed.is_(True))
        ) or 0
...
def learning_records(db: Session = Depends(get_db)) -> dict:
    rows = db.execute(
...
def me_vocab(
    ...
    query = (
        ...
        .where(UserVocabEntry.user_id == DEMO_USER_ID)
...
def me_favorites(page: int = Query(default=1, ge=1), size: int = Query(default=20, ge=1, le=50), db: Session = Depends(get_db)) -> dict:
    rows = db.execute(
        select(UserArticleFavorite, Article)
        .join(Article, Article.id == UserArticleFavorite.article_id)
        .where(UserArticleFavorite.user_id == DEMO_USER_ID, UserArticleFavorite.is_favorited.is_(True))
