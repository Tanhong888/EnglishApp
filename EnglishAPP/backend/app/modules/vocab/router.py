...
from app.db.models import Article, UserVocabEntry, Word
...
@router.post("")
def add_vocab(payload: AddVocabRequest, db: Session = Depends(get_db)) -> dict:
    ...
    return success(
        {
            ...
            "dedup_key": f"{DEMO_USER_ID}:{entry.word_id}:{entry.source_article_id}",
            "created": created,
        }
    )
...
def update_vocab(entry_id: int, payload: UpdateVocabRequest, db: Session = Depends(get_db)) -> dict:
    entry = db.scalar(
        select(UserVocabEntry).where(UserVocabEntry.id == entry_id, UserVocabEntry.user_id == DEMO_USER_ID)
    )
    if entry is None:
