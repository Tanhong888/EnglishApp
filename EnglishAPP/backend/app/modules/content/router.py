...
from app.db.models import Article, ArticleParagraph, UserArticleFavorite
...
def favorite_article(article_id: int, db: Session = Depends(get_db)) -> dict:
    article = db.get(Article, article_id)
    if article is None:
        raise HTTPException(status_code=404, detail="article not found")
    ...
    if favorite is None:
        favorite = UserArticleFavorite(user_id=DEMO_USER_ID, article_id=article_id, is_favorited=True)
        db.add(favorite)
    else:
...
def unfavorite_article(article_id: int, db: Session = Depends(get_db)) -> dict:
    favorite = db.scalar(
        select(UserArticleFavorite).where(
            UserArticleFavorite.user_id == DEMO_USER_ID,
            UserArticleFavorite.article_id == article_id,
