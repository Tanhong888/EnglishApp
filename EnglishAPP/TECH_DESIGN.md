...
- `GET /api/v1/me/vocab`（默认按词去重，支持 `q` 搜索词形/释义/词性，可按 `source_article_id` 过滤来源明细）
...
- 限流：登录默认 120 次/分钟（IP+邮箱双键）、查词默认 240 次/分钟（IP）、小测提交默认 120 次/分钟（IP）；`POST /api/v1/analytics/events` 默认 120 次/分钟（用户+IP 双键）
- 限流实现：后端采用统一滑动窗口限流组件，后续可平滑迁移到 Redis 分布式限流
- 安全响应头：统一追加 `X-Request-ID`、`X-Content-Type-Options`、`X-Frame-Options`、`Referrer-Policy`、`Permissions-Policy`，生产可开启 HSTS
- 异常追踪：HTTP 异常 / 校验异常 / 未捕获异常统一结构化返回，携带 `trace_id`（等于 `X-Request-ID`）便于日志关联
- 数据最小化：仅保留必要学习行为数据
...
- 提供 `GET /api/v1/web-articles/search`
- 服务端按配置聚合公开 RSS / Atom 英文内容源
- V1 仅返回标题、摘要、发布时间、来源和原文链接
- V1 不抓取全文正文、不做版权落库、不做站外网页清洗

### 17.2 查询参数

- `q`：可选，最少 2 个字符；为空时返回聚合后的最新结果
- `page`：默认 1
- `size`：默认 10，最大 20
...
- `WEB_ARTICLE_FEED_URLS`：逗号分隔的 RSS / Atom 源地址列表
- `WEB_ARTICLE_REQUEST_TIMEOUT_SECONDS`：抓取超时秒数

### 17.5 客户端策略

- 首页提供“在线英文文章搜索”入口
- 搜索结果页支持关键词查询、分页浏览、复制原文链接
- 当部分源失败时，前端展示告警但仍展示成功返回的结果
...
- V1 最小实现采用 `X-Admin-Key` 请求头
- 服务端通过 `ADMIN_API_KEY` 环境变量校验管理访问权限
- 草稿文章仅管理接口可见，前台 `GET /articles` 与 `GET /articles/{id}` 不返回未发布内容

### 18.3 当前覆盖范围

- 文章标题、阶段、等级、主题、阅读时长维护
- 段落全文录入与覆盖更新
- 发布 / 下线切换
...
## 19. 后台解析 / 小测 / 词汇管理（V1 增补）

### 19.1 管理接口

- `GET /api/v1/admin/articles/{article_id}/sentence-analyses`
- `PUT /api/v1/admin/articles/{article_id}/sentence-analyses`
- `GET /api/v1/admin/articles/{article_id}/quiz`
- `PUT /api/v1/admin/articles/{article_id}/quiz`
- `GET /api/v1/admin/words`
- `POST /api/v1/admin/words`
- `PATCH /api/v1/admin/words/{word_id}`

### 19.2 当前录入策略

- 重点句解析采用整篇覆盖写入，按 `sentence_index` 排序保存
- 小测采用整篇覆盖写入，题目内维护 `correct_option_index`
- 词汇管理支持分页查询、按关键词筛选、新增与编辑释义元数据

### 19.3 发布边界

- 草稿文章的解析与小测仅后台管理接口可见
- 前台公开接口仅在文章 `is_published=true` 时返回解析与小测

## 20. Mock TTS 任务流（当前实现）

### 20.1 当前接口

- `GET /api/v1/admin/articles/{article_id}/audio-task`
- `POST /api/v1/admin/articles/{article_id}/audio/generate`

### 20.2 当前行为

- 文章发布时自动创建或重置音频任务
- 应用内后台线程轮询任务表，推进 `pending -> processing -> ready | failed`
- 当前为 mock 生成器：成功时写入模拟音频 URL，失败关键字命中后按指数退避重试
- 超过最大重试次数后置为 `failed`

### 20.3 配置项

- `TTS_WORKER_ENABLED`
- `TTS_WORKER_POLL_INTERVAL_SECONDS`
- `TTS_PROCESSING_DELAY_SECONDS`
- `TTS_RETRY_BASE_DELAY_SECONDS`
- `TTS_MAX_ATTEMPTS`
- `TTS_MOCK_FAIL_KEYWORD`
- `PUBLIC_BASE_URL`

### 20.4 与正式云 TTS 的关系

- 当前实现主要用于打通 V1 任务流、状态机、失败重试与客户端降级
- 后续只需将任务执行器替换为真实云 TTS 调用与对象存储上传，即可平滑演进
